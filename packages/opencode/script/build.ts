...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/ratopai`,
      execArgv: [`--user-agent=ratopai/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    files: {
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/ratopai`
...
if (Script.release) {
  ...
  const debMap: Record<string, string> = {
    "ratopai-linux-x64": "amd64",
    "ratopai-linux-arm64": "arm64",
  }
